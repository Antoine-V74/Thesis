#!/usr/bin/env bash
# Job 16a - Pretrain Arm C3 (Hybrid: SupCon + SVDD compactness + Outlier Exposure).
# EXPLORATORY, most complex rung. Combines class structure (SupCon) with AD-native
# geometry (compact normal + danger pushed out), single 8 s scale. Public labels at
# PRETRAINING ONLY; deployment stays label-free. Same frozen scorer + gold exclusion.
...
CKPT_DIR="${CKPT_DIR:-Results/layer3/pretrain/supcon_hybrid_mitbih_seed0_100ep_8s_goldexcluded}"
...
echo "[OK] Arm C3 (supcon_hybrid) checkpoint dir -> $CKPT_DIR"
