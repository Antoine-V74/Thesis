#!/usr/bin/env bash
# Exploratory architecture: healthy VICReg with average+max temporal pooling.
set -euo pipefail
cd "$(dirname "$0")/../../.."

python Layer3/tools/pretrain_encoder.py \
  --window-index Results/layer3/window_index/layer3_windows_mitbih_8s_125hz.csv \
  --checkpoint-dir Results/layer3/pretrain/exploratory_vicreg_healthy_avgmax_seed0 \
