#!/usr/bin/env bash
# Job 14b - Phase 1 A0 + Arm C1 (supcon_oe). Requires: 14a finished (encoder_last.pt).
...
CKPT="${CKPT:-Results/layer3/pretrain/supcon_oe_mitbih_seed0_100ep_8s_goldexcluded/encoder_last.pt}"
OUT_DIR="${OUT_DIR:-Results/layer3/validation/pilot_mitbih_supcon_oe_seed0_100ep_8s}"

if [[ ! -f "$CKPT" ]]; then
  echo "[ERROR] Checkpoint not found: $CKPT - run 14a_pretrain_arm_c1_supcon_oe.sh first." >&2
  exit 1
fi
...
echo "[OK] Phase 1 A0+C1 outputs -> $OUT_DIR"
echo "Headline: false-permit DANGEROUS + bootstrap CI + CAV vs A0. Compare against A0, C, and the SSL arms."
