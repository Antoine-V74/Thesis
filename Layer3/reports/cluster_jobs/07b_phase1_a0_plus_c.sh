#!/usr/bin/env bash
# Job 7b — Phase 1 A0 + Arm C. Requires: 07a finished (encoder_last.pt).
...
CKPT="${CKPT:-Results/layer3/pretrain/supcon_mitbih_seed0_100ep_8s_goldexcluded/encoder_last.pt}"
OUT_DIR="${OUT_DIR:-Results/layer3/validation/pilot_mitbih_supcon_seed0_100ep_8s}"

if [[ ! -f "$CKPT" ]]; then
  echo "[ERROR] Checkpoint not found: $CKPT — run 07a_pretrain_arm_c_supcon.sh first." >&2
  exit 1
fi
...
echo "[OK] Phase 1 A0+C outputs -> $OUT_DIR"
echo "Headline: false-permit DANGEROUS + bootstrap CI + CAV vs A0."
echo "Compare against A0 and the SSL arms (ntxent / vicreg / mae_consistency)."
